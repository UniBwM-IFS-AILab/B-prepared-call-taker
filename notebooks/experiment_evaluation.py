...
def _():
    ...
    options = (
        sorted(path.name for path in experiments_root.iterdir() if path.is_dir())
...
def _(experiment_dropdown):
    ...
    experiment_path: Path = (
        Path("experiments") / "_missing" / "logs"
        if selected_experiment is None
        else Path("experiments") / selected_experiment / "logs"
    )
    list(experiment_path.rglob("survey.json"))[:3] if experiment_path.exists() else []
...
def _(experiment_dropdown):
    gt_dir = (
        Path("experiments") / str(experiment_dropdown.value) / "scenario_ground_truth"
    )
    gt_dir.absolute()
    return (gt_dir,)
...
def extract_scenario_name(path: Path):
    ...
    match = re.search(r"scenario_(\d+)", path.stem, flags=re.IGNORECASE)
    if match is None:
        raise ValueError(f"Could not extract scenario id from {path.name}")
...
def load_ground_truths(dir_path: Path):
    ...
    paths = sorted(dir_path.rglob("*.json"))
